...
XMFLOAT3 CGraphicsPipeline::ScreenTransform(XMFLOAT3& xmf3Project)
{
	...
	float fHalfWidth = m_pViewport->GetWidth() * 0.5f;
	float fHalfHeight = m_pViewport->GetHeight() * 0.5f;
	f3Screen.x = m_pViewport->GetLeft() + (xmf3Project.x * fHalfWidth) + fHalfWidth;
	f3Screen.y = m_pViewport->GetTop() + (-xmf3Project.y * fHalfHeight) + fHalfHeight;
