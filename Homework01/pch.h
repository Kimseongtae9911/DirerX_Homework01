...
namespace Vector3
{
	...
	//float3 와 scalar값을 받아 벡터를 정규화하여 scarlar값 계산 or vector에 scalar값을 곱하여 반환
	...
	//두 벡터가 이루는 각(vector) 정규화된 vector
	...
	//두 벡터가 이루는 각(float3) 정규화 안됨
	inline float Angle(XMFLOAT3& xmf3Vector1, XMFLOAT3& xmf3Vector2)
	{
		return(Angle((XMVECTOR&)XMVector3Normalize(XMLoadFloat3(&xmf3Vector1)), (XMVECTOR&)XMVector3Normalize(XMLoadFloat3(&xmf3Vector2))));
	}

	//float3벡터와 변환행렬을 입력받아 해당 변환행렬을 적용하고 정규화하여 반환
	...
	//float3벡터와 XMFLOAT4x4 변환행렬을 받아 정규화한 변환벡터를 반환
	inline XMFLOAT3 TransformNormal(XMFLOAT3& xmf3Vector, XMFLOAT4X4& xmmtx4x4Matrix)
	{
		return(TransformNormal(xmf3Vector, (XMMATRIX&)XMLoadFloat4x4(&xmmtx4x4Matrix)));
	}
	...
	{
		return(TransformCoord(xmf3Vector, (XMMATRIX&)XMLoadFloat4x4(&xmmtx4x4Matrix)));
	}
}
