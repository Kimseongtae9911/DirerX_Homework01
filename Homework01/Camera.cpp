...
{
	float fAspectRatio = (float(m_Viewport.GetWidth()) / float(m_Viewport.GetHeight()));
...
void CCamera::SetViewport(int nLeft, int nTop, int nWidth, int nHeight)
{
	...
	m_fAspectRatio = float(m_Viewport.GetWidth()) / float(m_Viewport.GetHeight());
}
...
void CCamera::Update(CPlayer* pPlayer, XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	mtxRotate._11 = pPlayer->GetRight().x; mtxRotate._21 = pPlayer->GetUp().x; mtxRotate._31 = pPlayer->GetLook().x;
	mtxRotate._12 = pPlayer->GetRight().y; mtxRotate._22 = pPlayer->GetUp().y; mtxRotate._32 = pPlayer->GetLook().y;
	mtxRotate._13 = pPlayer->GetRight().z; mtxRotate._23 = pPlayer->GetUp().z; mtxRotate._33 = pPlayer->GetLook().z;

	XMFLOAT3 xmf3Offset = Vector3::TransformCoord(pPlayer->GetCameraOffset(), mtxRotate);
	XMFLOAT3 xmf3Position = Vector3::Add(pPlayer->GetPos(), xmf3Offset);
	...
	if (fDistance > 0)
	{
		...
		SetLookAt(pPlayer->GetPos(), pPlayer->GetUp());
	}
}
